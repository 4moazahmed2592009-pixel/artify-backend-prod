...
app.post('/api/whop-webhook', async (req, res) => {
  try {
    ...
    try {
      await processedWebhooks.insertOne({
        _id: webhookId,
        created_at: new Date()
      });
    } catch (dbErr) {
      if (dbErr.code === 11000) {
        console.warn(
          `Webhook ${webhookId} already processed (Idempotency skip).`
        );
    ...
    const planId =
      ...
      (data.line_items && data.line_items[0]?.plan_id) ||
      '';
    ...
    if (
      ...
      if (
        existingUser &&
        existingUser.subscription_active &&
        existingUser.expires_at > now
      ) {
        newExpiry =
          existingUser.expires_at + durationMs;
      }
      ...
      await users.updateOne(
        { email },
        {
          $set: {
            ...
            plan: 'PRO',
            started_at:
              existingUser?.started_at &&
              existingUser.started_at > 0
                ? existingUser.started_at
                : now,
...
app.post('/api/claim-early-bird', csrfCheck, requireAuth, async (req, res) => {
  try {
    ...
    if (!ageConfirmed) {
      return res.status(403).json({
        error: 'يجب تأكيد أن عمرك 18 عاماً أو أكثر للمتابعة'
      });
    }
    ...
    if (updateResult.modifiedCount === 0) {
      ...
      if (
        coupon &&
        coupon.claimed_by &&
        coupon.claimed_by.includes(email)
      ) {
        return res.status(400).json({
          error: 'لقد استخدمت مكافأة أول 15 مستخدم من قبل'
        });
      }

      return res.status(400).json({
        error: 'عذراً، نفدت المقاعد المجانية المتاحة'
      });
    }
    ...
    res.json({
      success: true,
      message: 'تم تفعيل حساب PRO بنجاح!'
    });
...
app.post('/api/apply-coupon', csrfCheck, requireAuth, async (req, res) => {
  try {
    ...
    if (coupon.used_count >= coupon.max_uses) {
      return res.status(400).json({
        error: 'عذراً، نفدت المقاعد المجانية المتاحة لهذا الكود'
      });
    }
...
app.get('/api/promo-stats', async (req, res) => {
  try {
    ...
    if (!coupon) {
      return res.json({
        active: true,
        remaining: EARLY_BIRD_MAX_SEATS,
        currentRank: 1
      });
    }
    ...
    res.status(500).json({
      error: 'Failed to fetch promo stats'
    });
  }
});
...
// ==========================================
// تم إصلاح هذا المسار:
// كان يفتح TOOL_URL مباشرة بدون access_token.
// الآن ينشئ Access Token ثم يضيفه إلى رابط الأداة.
// ==========================================
...
app.get('/api/launch-app', async (req, res) => {
  try {
    ...
    // تحديد هل TOOL_URL يحتوي بالفعل على ?
    const separator =
      TOOL_URL.includes('?')
        ? '&'
        : '?';

    // إضافة access_token إلى رابط الأداة
    const toolUrlWithToken =
      `${TOOL_URL}${separator}access_token=${encodeURIComponent(accessToken)}`;

    // فتح الأداة بالرابط الصحيح
    return res.redirect(toolUrlWithToken);
...
// ==========================================
// الحصول على رابط الأداة مع Access Token
// ==========================================
...
app.listen(PORT, () => {
  console.log(
    `Server running securely on port ${PORT} (Prod Mode: ${isProd})`
  );
});
